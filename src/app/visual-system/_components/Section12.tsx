...
function VideoThumb({ poster, src }: { poster: string; src: string }) {
  ...
  // Start muted so the poster sits quietly until hovered.
  useEffect(() => {
    if (videoRef.current) videoRef.current.muted = true;
  }, []);

  // Hover = play WITH audio. Browsers block unmuted autoplay until the page has
  // a user gesture, so fall back to a muted preview if the sound play is denied.
  ...
  // Click guarantees audio (a click is always a valid user gesture).
  const enableSound = () => {
    const v = videoRef.current;
    if (!v) return;
    v.muted = false;
  ...
  const dir = parts.pop() || "section-12";
      ...
      >
        ...
        >
          {VIDEOS.map((n) => (
            <VideoThumb
              key={n}
              poster={`${TWELVE_BASE}/${n}.png`}
              src={`${TWELVE_BASE}/${n}.mp4`}
            />
          ))}
        </motion.div>
